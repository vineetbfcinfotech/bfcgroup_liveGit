...
//	staff_model->get('', ['active'=>1]);
	...
	public function department_name($d_id){
		$arrayId = explode(",", $d_id);
		//print_r($arrayId);
		$this->db->select("*");
		$this->db->from("tbldepartments");
		$this->db->where_in('departmentid', $arrayId);
		
		return $this->db->get()->result();
		//echo $this->db->last_query();exit;
	}
	public function team_name($t_id){
		$arrayId = explode(",", $t_id);
		$this->db->select("*");
		$this->db->from("tblteams");
		$this->db->where_in('id', $arrayId);
		return $this->db->get()->result();
	}
	public function role_name($r_id){
		$arrayId = explode(",", $r_id);
		$this->db->select("*");
		$this->db->from("tblroles");
		$this->db->where_in('roleid', $arrayId);
		return $this->db->get()->result();
	}
	public function reportings($rm_id){
		$arrayId = explode(",", $rm_id);
		$this->db->select("*");
		$this->db->from("tblstaff");
		$this->db->where_in('staffid', $arrayId);
		return $this->db->get()->result();
	}
    ...
    public function member($id = ''){
       ...
       //$id = 31;
            ...
            if ($this->input->post()) {
                ...
                $data['member_team']  = implode(",",$this->input->post('team_id'));
				$data['notice_date']              = $this->input->post('notice_date');
                $data['exit_date']              = $this->input->post('exit_date');
                $data['work_group']              = $this->input->post('work_group');
				if($id == ''){
					$data['active']              = '1';
				}else{
