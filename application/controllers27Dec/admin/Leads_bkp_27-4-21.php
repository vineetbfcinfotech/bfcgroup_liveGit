...
// echo $services."book".$book_type;die;
		...
		$html = $this->randerhtml($result, $packages,$services);
		echo $html;
	}

	public function randerhtml($result, $packages=NULL, $services){
		if(!empty($result)){
			if($services == 2){ ?>
			<div class="col-md-12">
		...
		<?php
		foreach($result as $data){ //print_r($data);exit;?>
		<div class="main-data">
			<div class="col-md-5">
			 <div class="form-group sub_service_data">
				<label class="checkbox-inline">
				<input type="checkbox" class="subservice_check <?php if($data->subservice_name == "Format Editing" ){ echo 'page_cost'; }elseif($data->subservice_name == "Proofreading"){ echo 'page_cost2'; } ?>" name="sub_services[]" data-cost="<?php echo $data->cost; ?>" data-id="<?php echo $data->id; ?>" value="<?php echo $data->id; ?>" <?php if($data->subservice_name == "Format Editing" ){ echo 'data-page-cost = "15"'; }elseif($data->subservice_name == "Proofreading"){ echo 'data-page-cost = "15"'; } ?> data-name="<?php echo $data->subservice_name; ?>" <?php if($data->cost == 0){ echo "checked"; } ?> <?php if($data->cost == 0){ ?>onclick='return false;' <?php } ?> >
			...
			<div class="col-md-4">
			 <div class="form-group">
				<div class="dropdown bootstrap-select form-control">
				   <input type="text" id="cost[]" name="cost" class="form-control <?php if($data->subservice_name == "Format Editing" ){ echo 'cost_page'; }elseif($data->subservice_name == "Proofreading"){ echo 'cost_page2'; } ?>" readonly  value="<?php echo $data->cost; ?>" >
				</div>
			 </div>
			</div>
		...
		$(document).ready(function() {
			var update_package_value = $('#package_value').val();
			...
			if(update_package_value != ""){
				var total_data = total+parseInt(update_package_value);
			}else{
    ...
    public function getservies_sc(){
            ...
            foreach($result as $data){
                ...
                $html .= "<ul>";
                    ...
                    foreach($subservices as $subservice){
                    $html .= "<li><input type='checkbox' name='sub_services[]' value='".$subservice->id."' class='subservicesstand_sc' id='myCheck".$subservice->id."' onclick='myFunction(".$subservice->id.",".$subservice->cost.");' data-service='".$data['service_name']."' data-name='".$subservice->subservice_name."' data-id='".$subservice->id."' data-cost='".$subservice->cost."' > ".$subservice->subservice_name."<input type='text' disabled value='".$subservice->cost."'</li>";
                    $servicecost = $subservice->cost;
                    }
                $html .= "</ul>";
         ...
         $result = $this->leads_model->getservies_coustize_standard($package, $book_type,$package_details);
            ...
            foreach($result as $data){
                ...
                $html .= "<ul>";
                    ...
                    $html .= " value='".$subservice->id."' class='".$i."' class='subservicesstand_sc' id='myCheck".$subservice->id."' onclick='myFunction(".$subservice->id.",".$subservice->cost.");' data-service='".$data['service_name']."' data-name='".$subservice->subservice_name."' data-id='".$subservice->id."' data-cost='".$subservice->cost."' > ".$subservice->subservice_name."<input type='text' disabled value='".$subservice->cost."'</li>";
                    $servicecost = $subservice->cost;
                  $i++;}
                $html .= "</ul>";
