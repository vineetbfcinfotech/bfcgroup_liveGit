       ...
       <thead>
        <tr role="row">
         ...
         foreach ($service as $key => $value_service) { 
          ...
          if ($service_name->service_name == 'Marketing your Masterpiece') { $ii++; 
            if ($ii == 1) {
...
$jj = 0;
        foreach ($service as $key => $value_service) {
          $service_name = $this->db->get_where('tblpackageservices',array('id'=>$value_service))->row();
          if ($service_name->service_name == 'Marketing your Masterpiece') { $jj++;
          if ($jj== 1 ) {
          ?>
            <td>
             <?php if ($all_data->lead_raw_ms=='') { ?>
...
<td>
 <form method="post" action="<?php echo base_url(); ?>admin/Pm_lead/binding"> 
  <div class="form-group">
   <input type="hidden" name="hidden_id" id="hidden_id" class="hidden_id" value="<?php echo $all_data->id; ?>"/>
   <label>Binding</label>
   <select class="form-control" id="binding" name="binding" >
    <option>-select-</option>
    <option <?php if ($all_data->book_cover_sc == 'MultiColor 250 GSM') { echo "Selected"; }?> value="MultiColor 250 GSM">MultiColor 250 GSM</option>
    <option <?php if ($all_data->book_cover_sc == 'Hardbound') { echo "Selected"; }?> value="Hardbound">Hardbound</option>
   </select>
</div>
<div class="submit-block">
  <div class="form-group">
