...
if (payment_status==1) {
	$(document).on('click', '#submit_booking_amount', function() {
		...
		var data = $('#received_booking_amount').val();
			...
			// alert('payment_id'+ payment_id + 'payment_status'+payment_status+ 'data'+data+ 'author'+author+ 'book'+book);
			// console.log('payment_id'+ payment_id + 'payment_status'+payment_status+ 'data'+data+ 'author'+author+ 'book'+book);
			...
			$.ajax({
				...
				data: {'payment_id': payment_id, 'payment_status':payment_status, 'data':data, 'author':author, 'book':book, 'tbltype':tbltype },
