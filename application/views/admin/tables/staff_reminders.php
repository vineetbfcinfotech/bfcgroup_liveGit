...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'tblreminders.date') {
            ...
            $CI = & get_instance();
            $CI->db->select('*');
            $CI->db->where('lead_id_start <=', $aRow['rel_id']);
            $CI->db->where('lead_id_end >=', $aRow['rel_id']);
            $res = $CI->db->get('tblleads_custom')->row();
            $lead_id_start = $res->lead_id_start;
            $lead_id_end = $res->lead_id_end;
            $name = $res->name;
            $page1 =$aRow['rel_id'] - $lead_id_start;
            $page = floor($page1/100)*100;
            $CI->db->select('source');
            $CI->db->where('id',$aRow['rel_id']);
            $leresnam = $CI->db->get('tblleads')->row();
            $leadsour = $leresnam->source;
            if($leadsour == 2)
            {
            ...
            {
                $basehref = base_url() . "admin/leads/viewassignedleads/";
            }
            
            
            $_data      = '<a href="' . $basehref. '">' . $rel_values['name'] . '</a>';
