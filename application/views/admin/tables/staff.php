...
array_push($join,'LEFT JOIN tblstaff as b ON b.staffid=tblstaff.reporting_manager');
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'tblstaff.last_login') {
            if ($_data != null) {
            ...
            $_data .= ' <a href="' . admin_url('staff/member/' . $aRow['staffid']) . '">'.$aRow['tblstaff.firstname'].' '.$aRow['lastname']. '</a>';
            ...
            $_data .= '</div>';
        } elseif ($aColumns[$i] == 'tblstaff.email') {
