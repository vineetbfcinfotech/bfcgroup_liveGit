...
class Leadmodel_chfilter extends CRM_Model
{
...
// $this->db->where('id', $id);
     ...
     function uploadleadsbyPc()
    {
        ...
        //    set_alert('success', _l('import_total_imported', $total_imported.' '.$total_data));
        }
        // echo $lead_id;
        //  print_r($data);exit;
...
// return  "test".$services.$book_type;die;
     ...
     public function filter_by_assignlead()
    {
       ...
       // echo $stf[0];
        ...
        if (isset($end_date) && $end_date != null  ) 
        {
            ...
            $this->db->select('tblleadsstatus.name,tblleads.craete_package,tblleads.id,tblleads.otherphonenumber,tblleads.lead_booktitle,tblleads.lead_bookformat,tblleads.description,tblleads.calling_objective,tblleads.assigned,tblleads.status,tblleads.lead_created_date,tblleads.email,tblleads.next_calling,tblleads.lead_adname,tblleads.lead_author_msstatus,tblleads.lead_author_name,tblleads.phonenumber,tblleads.created_at,tblleads.lead_ad_id,tblleads.lead_author_mslanguage,tblleads.leadId,tblleads.lead_category_id,tblleads.lead_publishedearlier,tblleads.lead_callingdate,tblleads.lead_reviewstatus,tblleads.lead_approve_current_date,tblleads.ImEx_leadRemarks,tblleads.created_at,tblleads.next_calling');
 ...
 if (isset($start_date) && $start_date != '' && isset($end_date) && $end_date != ''  ) 
        {
            // $this->db->where('lead_created_date >= "'.$start_date.'" AND lead_created_date <= "'.$end_date'"');
            // $this->db->where('lead_created_date >=',$start_date);
    ...
    public function updateData($id,$name,$phone,$msstatus,$earlier,$language){
        $data = array(
            'lead_author_name' => $name,
            'phonenumber'  => $phone,
            'lead_author_msstatus'  => $msstatus,
            'lead_publishedearlier' => $earlier,
            'lead_author_mslanguage' => $language
        );
        $this->db->where('id', $id);
        $this->db->update('tblleads', $data);
        return true;
...
$staff_id = $_SESSION['staff_user_id'];
 ...
 $task_type = $_POST['tasktypefilter'];
     ...
     public function get_row_count(){
        ...
        $this->db->select('id,lead_acquired_date,lead_author_name,lead_package_name,lead_bookformat,lead_packge_value,lead_packg_totalamount')->from('tblleads');
        
        if($this->session->userdata('s_date')){
            $query = $this->db->where('lead_acquired_date >= "'. date('Y-m-d', strtotime($this->session->userdata('s_date'))). '" ')->get(); 
        }elseif($this->session->userdata('e_date')){
            $query = $this->db->where('lead_acquired_date >= "'. date('Y-m-d', strtotime($this->session->userdata('e_date'))). '" ')->get(); 
        }elseif( ($this->session->userdata('s_date')) && ($this->session->userdata('e_date')) ){
            $query =  $this->db->where('lead_acquired_date BETWEEN "'. date('Y-m-d', strtotime($this->session->userdata('s_date'))). '" and "'. date('Y-m-d', strtotime($this->session->userdata('e_date'))).'"')->get();
        }
        else{
            // return redirect(admin_url().'AcquisitionReport/aquisition_report', 'refresh');
            $query = $this->db->get();
        }

        return $query->num_rows(); 
     }
     public function sel_query_by_session($config,$page){
        $this->load->library('session');
        
        $this->db->select('id,lead_acquired_date,lead_author_name,lead_package_name,lead_bookformat,lead_packge_value,lead_packg_totalamount');
        $this->db->limit($config, $page);
        if($this->session->userdata('s_date')){
            $this->db->where('lead_acquired_date >= "'. date('Y-m-d', strtotime($this->session->userdata('s_date'))). '" ')->get(); 
        }elseif($this->session->userdata('e_date')){
           $this->db->where('lead_acquired_date >= "'. date('Y-m-d', strtotime($this->session->userdata('e_date'))). '" ')->get(); 
        }elseif( ($this->session->userdata('s_date')) && ($this->session->userdata('e_date')) ){
            $this->db->where('lead_acquired_date BETWEEN "'. date('Y-m-d', strtotime($this->session->userdata('s_date'))). '" and "'. date('Y-m-d', strtotime($this->session->userdata('e_date'))).'"')->get();
        }
        else{
            // return redirect(admin_url().'AcquisitionReport/aquisition_report', 'refresh');
            $query = $this->db->get('tblleads');
...
//  row count aquisition test

     public function get_row_countTest($start_date = '',$end_date='',$staff_name=''){
 ...
 if (isset($start_date) && $start_date != '' && isset($end_date) && $end_date != ''  ) 
        {
            ...
            $this->db->where('lead_acquired_date BETWEEN "'.$start_date.'" and "'. $end_date.'"');
        }
     ...
     public function get_row_dataTest($limit, $start,$start_date = '',$end_date='',$staff_name=''){
        $this->load->database();
        $this->db->select('id,lead_acquired_date,lead_author_name,lead_package_name,lead_bookformat,lead_packge_value,lead_packg_totalamount,assigned');
        $this->db->limit($limit, $start);
        if($staff_name != ''){
 ...
 if (isset($start_date) && $start_date != '' && isset($end_date) && $end_date != ''  ) 
     ...
     // get data for export data
     public function exportleadData($limit, $start,$search_text='',$search_cat='',$start_date = '',$end_date='', $staff_name = ''){
        $role = get_imp_role();
        $arr = herapermission();
        $arr = explode(',', $arr);
         $useraid = $this->session->userdata('staff_user_id');
         $this->load->database();
         $this->db->select('id,otherphonenumber,lead_booktitle,lead_bookformat,name,description,calling_objective,assigned,status,lead_created_date,email,next_calling,lead_adname,lead_author_msstatus,email,lead_author_name,phonenumber,created_at,lead_ad_id,lead_author_mslanguage,leadId,lead_category_id,lead_publishedearlier,lead_callingdate,lead_reviewstatus,ImEx_NextcallingDate,lead_approve_current_date,ImEx_leadRemarks,ImEx_CreatedAt');
         if (!(is_admin())) {
 ...
 if (isset($start_date) && $start_date != '' && isset($end_date) && $end_date != ''  ) 
     ...
     //page count for exportlead
     public function exportleadCount($search = '',$search_cat='',$start_date = '',$end_date='',$staff_name = '') {
        $id = $this->session->userdata('staff_user_id');
        $role = get_imp_role();
        $arr = herapermission();
        $arr = explode(',', $arr);
 ...
 if (isset($start_date) && $start_date != '' && isset($end_date) && $end_date != ''  ) 
    ...
    public function acquisitionData1($limit, $start,$pc_name='',$from_date = '',$to_date=''){
    ...
    if (is_admin() || is_headtrm() || $staff_id==34 || $staff_id==28) {
        if($from_date != ''){
            $this->db->where('lead_acquired_date >=', $from_date."00:00:00.000");
        }
        if($to_date != ""){
            $this->db->where('lead_acquired_date <=', $to_date."23:59:59.997");
        }
        if($pc_name != ''){
            if($pc_name !='0'){
    ...
    public function acquisitionCount1($pc_name='',$from_date = '',$to_date='') {
       $staff_id = $this->session->userdata('staff_user_id');
       $this->db->select('count(*) as allcount');
       $this->db->from('tblleads');
   ...
   public function acquisitionData($limit, $start,$pc_name='',$from_date = '',$to_date=''){
...
$result1  = $this->db->get('tblleads_create_package')->result();
    ...
    // NOV 13,2021
    //page count for acquisition
    public function acquisitionCount($pc_name='',$from_date = '',$to_date='') {
       $staff_id = $this->session->userdata('staff_user_id');
       $role = get_imp_role();
       $arr = herapermission();
       $arr = explode(',', $arr);
    ...
    //    print_r($this->db->last_query());die;
    //    print_r$result[0]['allcount']+$result1[0]['allcount']);die;
