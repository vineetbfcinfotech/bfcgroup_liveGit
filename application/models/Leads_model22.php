...
//  $this->db->where('assigned_id',$loginid);
            ...
            $this->db->join('tblstaff AS tbll', 'tblleads_custom.assigned_id=tbll.staffid');
...
// $this->db->where('id', $id);
    ...
    function upload_leads()
    {
        ...
        $assiged = $this->input->post('responsible');
        ...
        else
        {
                ...
                set_alert('warning', "Lead Assigned Name is Missing");
        ...
        while ($csv_line = fgetcsv($fp, 10240)) {
            ...
            for ($i = 0, $j = count($csv_line); $i < $j; $i++) {
                if (!empty($csv_line[1])) {
                    $insert_csv = array();
                    $insert_csv['name'] = preg_replace('/[^ \w]+/', '', $csv_line[0]);
                    $insert_csv['phonenumber'] = preg_replace('/[^0-9]/', '', $csv_line[1]);
                    $insert_csv['designation'] = preg_replace('/[^ \w]+/', '', $csv_line[2]);
                    $insert_csv['company'] = preg_replace('/[^ \w]+/', '', $csv_line[3]);
                    $insert_csv['address'] = preg_replace('/[^ \w]+/', '', $csv_line[4]);
                    $insert_csv['email'] = preg_replace('/[^ \w]+/', '', $csv_line[5]);
                    $insert_csv['data_source'] = preg_replace('/[^ \w]+/', '', $csv_line[6]);
                    $insert_csv['calling_objective'] = preg_replace('/[^ \w]+/', '', $csv_line[7]);
                }
            }
            $i++;
            $total_imported++;
            $data = array(
                ...
                'data_source' => $insert_csv['data_source'],
                'calling_objective' => $insert_csv['calling_objective'],
                'status' => $status,
                'source' => $source,
                'assigned' => $assiged
            );
            $data['crane_features'] = $this->db->insert('tblleads', $data);
