...
class Announcements_model extends CRM_Model
{
	...
	public function save_custom_notification($data){
		$result = $this->db->insert('tblnotifications',$data);
		return $result;
	}
	public function get_custom_notification(){
		$this->db->select("*");
		$this->db->from("tblnotifications");
		$this->db->order_by("id", "desc");
		$this->db->where('description', 'custom_notification');
		$query = $this->db->get();
		$result = $query->result();
		return $result;
	}
