...
class Utilities_model extends CRM_Model
{
    ...
    public function get_calendar_data($start, $end, $client_id = '', $contact_id = '', $filters = false)
    {
        ...
        if (get_option('show_holiday_on_calendar') == 1 ) {
            $this->db->select('name,quota,DATE_ADD(leave_end_date, INTERVAL 1 DAY) AS enddate,id ' );
            $this->db->from('tblholidays');
            $this->db->where('(leave_end_date > "' . date('Y-m-d') . '" AND leave_end_date IS NOT NULL AND leave_end_date BETWEEN "' . $start . '" AND "' . $end . '" OR quota >"' . date('Y-m-d') . '")');
            ...
            foreach ($holidays as $holiday) {
                ...
                //$name = "holi";
                $_holiday['title']    = $name;
                $_holiday['color']    = get_option('calendar_holiday_color');
                $_contract['_tooltip'] = _l('calendar_contract') . ' - ' . $name . $rel_showcase;
                $_holiday['url'] = site_url('admin/leave');
                $_holiday['start'] = $holiday['quota'];
                $_holiday['end'] = $holiday['enddate'];
                
                array_push($data, $_holiday);
            }
        }
