...
$servicess = implode(", ",  $filteredArray);
	...
	{
         if (is_admin() || is_headtrm()) {
	$data['project_data'] = $this->db->order_by("createPackEmailDate", "desc")->get_where('tblleads',array('createPackEmailStatus'=>1,'createPackEmailDate !='=>''))->result();
}else{
     $staff_id = $_SESSION['staff_user_id'];
     $data['project_data'] = $this->db->order_by("createPackEmailDate", "desc")->get_where('tblleads',array('createPackEmailStatus'=>1,'createPackEmailDate !='=>'', 'assigned'=>$staff_id ))->result();
}
    ...
    $staff_id = $_SESSION['staff_user_id'];
    $data['project_data'] = $this->db->order_by("id", "desc")->get_where('tblleads',array('status'=>39 ,'assigned'=>$staff_id))->result();
}
$this->db->select('staffid,firstname');
                      $this->db->where('department_id',24);
                      $this->db->where('active',1);
