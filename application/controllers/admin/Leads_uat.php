...
class Leads_uat extends Admin_controller
...
//      print_r($data['package_data']);
 ...
 public function getservies_sc_for_edit($value='')
    ...
    /*  $message .= "<p>Warm Regards.</p>";*/
        ...
        $this->email->cc('shivangiyadav@bfcpublications.com');
        $this->email->cc('gaurav@bfcpublications.com');
    ...
    $staff_id = $_SESSION['staff_user_id'];
    $data['project_data'] = $this->db->select('id,createPackEmailDate,lead_author_name,lead_package_detail,lead_acquired_date,lead_adname,lead_author_mslanguage,lead_book_type,lead_packg_totalamount')->order_by("id", "desc")->get_where('tblleads',array('lead_category_id'=>39 ,'createPackEmailStatus'=>1,'assigned'=>$staff_id))->result();
}
$this->db->select('staffid,firstname');
                      $this->db->where('department_id',24);
                      $this->db->where('active',1);
    ...
    public function send_php_mail()
        {
                ...
                $mail->Password = 'EnqBFC2121';
                $mail->SMTPSecure = 'tls';
                $mail->Port     = 587;
                
                $mail->setFrom('enquiry@bfcpublications.com', 'BFC Publications');
                $mail->addReplyTo('enquiry@bfcpublications.com', 'BFC Publications');
                
                // Add a recipient
                $mail->addAddress('mayankm738@gmail.com');
