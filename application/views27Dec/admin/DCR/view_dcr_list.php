                ...
                <div class="panel_s">
                    <div class="panel-body">
                       <?php $converted_by = $this->session->userdata('staff_user_id'); 
                        ...
                        <div class="dropdown bootstrap-select show-tick">
                            <input type="button" class = "btn btn-primary" onclick="window.location.reload();" value="Reload">
                        </div>
					</div>	
				</div>	
			</div>	
                            ...
                            <table id="testTable" summary="Code page support in different versions of MS Windows." rules="groups" frame="hsides" border="2" class="table table-bordered scroll-responsive">
                                ...
                                <tbody  id="tblResult">
									 ...
									 if (is_admin()){
                                        ...
                                        <tr>
                                            ...
                                            <?php foreach ($data_count as $key => $data_c_a) { 
                                                $getData_all =   $this->db->select('assigned,COUNT(*) as cnt')->get_where('tblleads',array('assigned'=>$value->staffid,'lead_category_id'=>$data_c_a))->result_array(); 
                                            $all[$i] += $getData_all[0]['cnt'];
                                       }?>
                                            <td class="bold"><?=$all[$i];?></td>
                                            <?php $j=0; foreach ($data_c as $key => $data_c_a) {
                                                   if ($data_c_a  == 32) {
                                                    $getData_np =   $this->db->select('assigned,COUNT(*) as cnt')->get_where('tblleads',array('assigned'=>$value->staffid,'lead_category_id'=>$data_c_a))->result_array();  
                                  ...
                                  if ($id == $value['staffid']) {
                                        ...
                                        <?php foreach ($data_c as $key => $data_c_a) {
                                                   if ($data_c_a  == 32) {
                                                    $getData_np =   $this->db->select('assigned,COUNT(*) as cnt')->get_where('tblleads',array('assigned'=>$staff_list[0]['staffid'],'lead_category_id'=>$data_c_a))->result_array();  
                                             ...
                                             <td class="bold"><?=$full_count;?></td>
                                        <?php foreach ($data_c as $key => $data_c_a) {
                                         $getData =   $this->db->select('assigned,COUNT(*) as cnt')->get_where('tblleads',array('assigned'=>$staff_list[0]['staffid'],'lead_category_id'=>$data_c_a))->result_array(); ?>
                                        <td class="bold"><?php echo($getData[0]['cnt'])?></td>
                                      <?php } ?>
