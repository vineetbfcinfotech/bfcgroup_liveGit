         ...
         <?php foreach($data['result'] as $_result){
            ...
            switch($data['type']){
                ...
                case 'leads':
                    ...
                    $this->db->select('source,assigned');
                    $this->db->where('id',$_result['id']);
                    $sourceres = $this->db->get('tblleads')->row();
                    $source = $sourceres->source;
                    $assignedid = $sourceres->assigned;
                    
                    $this->db->select('CONCAT(tblstaff.firstname, " ", tblstaff.lastname) AS  assname');
                    $this->db->where('staffid', $assignedid);
                    $assignres = $this->db->get('tblstaff')->row();
                    $assigned = $assignres->assname;
                    
                    if($source  == 2)
                    {
                    $this->db->select('*');
                    $this->db->where('lead_id_start <=', $_result['id']);
                    $this->db->where('lead_id_end >=', $_result['id']);
                    $res = $this->db->get('tblleads_custom')->row();
                    $lead_id_start = $res->lead_id_start;
                    $lead_id_end = $res->lead_id_end;
                    $name = $res->name;
                    $page1 =$_result['id'] - $lead_id_start;
                    $page = floor($page1/100)*100;
                    
                    $basehref = base_url() . "admin/leads/view_custom_lead/" . $name . "/" . $lead_id_start . "/" . $lead_id_end . "/" . $page;
					
					
                    //$output = '<a href="'.admin_url('leads/view_custom_lead/'.$res->name.'/'.$res->lead_id_start.'/'.$res->lead_id_end).'">'.$_result['name'].'</a>';
                   // $output = '<a href="#" onclick="init_lead('.$_result['id'].');return false;">'.$_result['name'].' - '.$res->name.' - '.date('d M, Y', strtotime($_result['lastcontact'])).' - '.$res->lead_id_start.'- '.$res->lead_id_end.' - '.$_result['id'].' - '.$page.'</a>';
                    $output = '<a href="'.$basehref.'" >'.$_result['name'].' - '.$name.'  - '.$assigned.'</a>';
                    ...
                    {
                         $output = '<a href="'.admin_url('leads/viewassignedleads/').'">'.$_result['name'].'- Added Leads - '.$assigned.'</a>';
                    }
                break;
                case 'tasks':
