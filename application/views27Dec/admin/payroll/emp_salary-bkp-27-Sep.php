...
foreach ($staff_members as $v_employee)
  // echo $v_employee['firstname'] . ' ' . $v_employee['lastname'];
{ ?>
                            <tr>  
                              ...
                              // echo "SELECT `LogDate`, `UserId`, count(UserId) as cnt FROM $table WHERE month(LogDate) = $month AND `UserId` = '$empbioidmonth' GROUP BY `UserId`, date(LogDate) HAVING cnt > '2'";

                              $morecountinmonth = $this->db->query("SELECT `LogDate`, `UserId` FROM $table WHERE month(LogDate) = $month AND `UserId` = '$empbioidmonth'");
                              $morecounts =  $morecountinmonth->result();
                              // echo count($morecounts);
                              if(count($morecounts) > 2)
                              {
                              ...
                              $totaldaymissed = $this->db->query("SELECT `LogDate`, `UserId` FROM $table WHERE month(LogDate) = $month AND `UserId` = '$empbioidmonth'");
                             ...
                             //print_r($this->db->last_query());
                              ...
                              $date_l = $year."-".$month."-01";
                              $date_E = $year."-".$month."-31";
                              $this->db->select_sum('duration');
                              $this->db->where('user_id',$v_employee['staffid']);
                              $this->db->where('leave_start_date >=' ,$date_l);
                              // $this->db->where('leave_end_date <=' ,$date_E);
                              $this->db->where('application_status', '2');
                              $this->db->where('leave_category_id', '0');
                              $spl = $this->db->get('tblleaveapplication')->row();
                              // echo $this->db->last_query();
                              $SLleaves = $spl->duration;
                             ...
                             $totaldayded = $totallwpabsent+($acthldcount/2)-$totallwp;
                         ...
                         <td><?= $apphalf; ?></td>
                         <td><?= $acthldcount; ?></td>
                         <td data-staffid="<?= $curentstafid; ?>" data-bioid="<?= $empbioidmonth;  ?>" data-monthdate="<?= $reportmonthdate; ?>" class="daydeduction"><?= $totallwp; ?> </td>
                         <?php $this
        ->db
        ->select('net_salary');
    $this
...
?>
                         <td> 
                          <? $totalded1 = ($perdaysal * $totallwp);
                              echo $totalded = number_format((float)($totalded1) , 2, '.', ''); 
                          ?> </td>
                         <td> <?=$monthsalemp = number_format((float)($empsal-$totalded1) , 2, '.', ''); ?> </td>
                            </tr> 
