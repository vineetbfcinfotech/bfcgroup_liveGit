      ...
      <div class="panel_s">
       ...
       <hr class="hr-panel-heading" />
          ...
          <div class="profile mtop20 display-inline-block">
            ...
            <p><i class="fa fa-phone-square"></i><a href="tel:<?php echo $staff_p->phonenumber; ?>"> <?php echo $staff_p->phonenumber; ?></a></p>
            <?php } ?>
            <?php if($staff_p->alternate_phonenumber != ''){ ?>
            <p><i class="fa fa-phone-square"></i> <a href="tel:<?php echo $staff_p->alternate_phonenumber; ?>"> <?php echo $staff_p->alternate_phonenumber; ?></a></p>
            <?php } ?>
            
            <p><i class="fa fa-street-view"></i> <?php echo $staff_p->street_address; ?></p>
            <p><i class="fa fa-address-book-o"></i> <?php echo $staff_p->city; ?></p>
            <p><i class="fa fa-address-card"></i> <?php echo $staff_p->postal_code; ?></p>
           ...
           <div class="form-group mtop10">
              <label for="departments" class="control-label"><?php echo "Designation" ?></label>
              <div class="clearfix"></div>
              <?php
              foreach($departments as $department){ ?>
              <?php
              foreach ($staff_departments as $staff_department) {
               if($staff_department['departmentid'] == $department['departmentid']){ ?>
               <div class="chip-circle"><?php echo $staff_p->role; ?></div>
               <?php }
             }
             ?>
             <?php } ?>
  ...
  <!--<?php if ($staff_p->staffid == get_staff_user_id()){ ?>
   <div class="col-md-7">
    <div class="panel_s">
      <div class="panel-body">
       <h4 class="no-margin">
        <?php echo _l('staff_rm_to_tm_list'); ?>
       </h4>
       <hr class="hr-panel-heading" />
        <table class="table">
        <thead>
        ...
        <tbody>
        <?php if (!empty($telerms))  { ?>
          <?php foreach ($telerms as $key =>$one) : ?>
            <tr>
              <td><?= $one->full_name ?></td>
            </tr>
        <?php endforeach; ?> 
...
<script>
  $(function(){
   ...
   if(notifications.length > 0){
	...
	if(obj.description == "birthday_notification"){
		notifications += "Today is "+obj.from_fullname+ " Birthday, Write a timeline";
       }else if(obj.description == "new_joining"){
		   notifications += obj.from_fullname + ' - ';
			notifications += "New Join In BFC Team";
	   }else if(obj.description == "inactive"){
		   notifications += obj.from_fullname + ' - ';
			notifications += "Resign From Date";
	   }else{
		   notifications += obj.from_fullname + ' - ';
