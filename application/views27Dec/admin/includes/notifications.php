                        <a href="#" class="dropdown-toggle notification_data_status notifications-icon" data-toggle="dropdown" aria-expanded="false"><span class="label label-pill label-danger count" style="border-radius:10px;"></span>
                        ...
                        <ul class="dropdown-menu notifications animated fadeIn width400" data-total-unread="<?php echo $current_user->total_unread_notifications; ?>">
                          ...
                          <li class="relative notification-wrapper" data-notification-id="<?php echo $notification['id']; ?>">
                            ...
                            <a href="<?php echo admin_url($notification['link']); ?>" class="notification-top notification-link">
							  ...
							  /* if($notification['description'] == "custom_notification"){
								  echo '<img src="http://bfccrm-uat.ga/assets/images/user-placeholder.jpg" class="client-profile-image-small img-circle pull-left notification-image">';
							  } */
                              ?>
                              <div class="media-body">
                                ...
                                //$description = _l($notification['description'])."cn";
                                if(($notification['fromcompany'] == NULL && $notification['fromuserid'] != 0) || ($notification['fromcompany'] == NULL && $notification['fromclientid'] != 0)){
                                 /* if($notification['fromuserid'] != 0){
                                  $description = $notification['from_fullname']. ' - ' . $description;
                                } */
								//print_r($notification['description']);
								if($notification['description'] == "birthday_notification"){
                                  $description = "It's ".$notification['from_fullname']. " Birthday.";
                                }
								elseif($notification['description'] == "new_joining") 
								{
									$description = $notification['from_fullname']. " has joined as position in (company name)"." from ". $notification['join_date'];
								}
								elseif($notification['description'] == "inactive")
								{
									$description = $notification['from_fullname']. " will resign w.e.f ".$notification['join_date'];
								}
								elseif($notification['description'] == "custom_notification")
								{
									$description = $notification['custom_subject'];
								}
								else
								{
