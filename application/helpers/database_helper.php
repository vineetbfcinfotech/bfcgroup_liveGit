<?php
   ...
   function get_option( $name )
   {
      ...
      //print_r($CI->app->get_option($name));exit;
   ...
   function rmlist( $selected = 0 )
   {
      $CI = &get_instance();
      $rmLists = '<option value="0">--SELECT--</option>';
      $CI->load->model('teams_model');
      $userList = $CI->teams_model->rmuserlist();
      foreach ($userList as $list) {
         $rmLists .= sprintf('<option value="%s" %s> %s </option>', $list->id, $list->id == $selected ? 'selected' : '', $list->name);
      }
      return $rmLists;
   }
   
   function rander_getWorkGroup($staffID){
		$CI =&get_instance();
		$CI->db->select("*");
		$CI->db->from("tblstaff");
		$CI->db->where("staffid", $staffID);
	...
	function rander_getWfhDate($user_id){
		...
		//$CI->db->or_where('end_date >= ',$currentDate);
		$CI->db->order_by("id", "desc");
		$CI->db->limit(1);
		/* $query = $CI->db->query("SELECT * FROM tbl_wfh where 'staffid' = $user_id and 'status' = 1 ORDER BY id DESC LIMIT 1"); */
	...
	function rander_getLeave($staffs){
		$CI =&get_instance();
		$today = date("Y-m-d");
		$staffArray = explode(",",$staffs);
         
		$CI->db->select('tblleaveapplication.*, CONCAT(tblstaff.firstname," ",tblstaff.lastname) as full_name, tblstaff.staffid');
		$CI->db->from('tblleaveapplication');
		$CI->db->where('tblstaff.active', '1');
		if($staffs != 1){
			$CI->db->where_in('tblstaff.staffid', $staffArray);
		}
		
		$CI->db->where("tblleaveapplication.leave_start_date >=", $today);
		$CI->db->join('tblstaff', 'tblleaveapplication.user_id = tblstaff.staffid');
		$CI->db->order_by("tblleaveapplication.leave_application_id", "desc");
		$data = $CI->db->get()->result();
		//return $CI->db->last_query();
		return $data;
	}
	function rander_getLeaveCategory($catId){
		$CI =&get_instance();
		$CI->db->where('leave_category_id',$catId); 
		return $CI->db->get('tblleavecategory')->result();
	}
	
	function rander_getWfh($staffs){
		$CI =&get_instance();
		$today = date("Y-m-d");
		
		$staffArray = explode(",",$staffs);
		
// 		$CI->db->select("tbl_wfh.*, CONCAT(tblstaff.firstname,".",tblstaff.lastname) as full_name");
// 		$CI->db->from('tbl_wfh');
// 		$CI->db->where('tblstaff.active', '1');
...
// 			$CI->db->where_in('tblstaff.staffid', $staffArray);
// 		}
// 		$CI->db->where("tbl_wfh.start_date >= ", $today);
// 		//$CI->db->or_where('tbl_wfh.end_date <= ',$today);
		
// 		$CI->db->join('tblstaff', 'tbl_wfh.staffid = tblstaff.staffid');
// 		$CI->db->order_by("tbl_wfh.id", "desc");
        if($staffs == 1){
           $where  ='WHERE `tblstaff`.`active` = "1" AND (`tbl_wfh`.`start_date` >= "'.$today.'" OR `tbl_wfh`.`end_date` >= "'.$today.'")'; 
        }else{
            $where  ='WHERE `tblstaff`.`active` = "1" AND (`tbl_wfh`.`start_date` >= "'.$today.'" OR `tbl_wfh`.`end_date` >= "'.$today.'") AND tbl_wfh.end_date IN('.$staffs.')';
        }
        //echo $where;exit;
        $sql = $CI->db->query('SELECT `tbl_wfh`.*, CONCAT(tblstaff.firstname, tblstaff.lastname) as full_name FROM `tbl_wfh` JOIN `tblstaff` ON `tbl_wfh`.`staffid` = `tblstaff`.`staffid` '.$where.' ORDER BY `tbl_wfh`.`id` DESC');
	...
	function get_confirmation_date_satffwise($staffId){
		$CI =&get_instance();
		$CI->db->select("confirmation_date");
		$CI->db->from("tblstaff");
		$CI->db->where("staffid", $staffId);
